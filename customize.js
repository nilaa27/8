document.addEventListener('DOMContentLoaded', function() {
    ...
    const customizationOptions = {
        logo: {
            lang: 'ENG',
            text: 'pictlord',
            color: '#000000' // Default warna teks logo
        },
        date: {
            show: false,
            showTime: false
    ...
    const logoLangContainer = document.getElementById('logo-lang-container'); // Pastikan Anda menggunakan ID ini di HTML
    const logoColorPicker = document.getElementById('logoColorPicker');
    const dateCheckbox = document.getElementById('dateCheckbox');
    const timeCheckbox = document.getElementById('timeCheckbox'); // Perbaiki ID dari dateTimeCheckbox
    // --- MODIFIKASI SELESAI ---
    ...
    // SEMUA BLOK EVENT LISTENER STIKER ANDA TETAP SAMA DAN TIDAK DIUBAH
    ...
    // SEMUA BLOK EVENT LISTENER WARNA & GAMBAR FRAME ANDA TETAP SAMA DAN TIDAK DIUBAH
    ...
    // ... dan semua listener gambar frame lainnya tetap sama ...
    ...
    // --- MODIFIKASI DIMULAI ---
    // Logika untuk tombol logo dan color picker yang terintegrasi.
    // Pastikan Anda menambahkan atribut data-lang di HTML:
    // <button id="engLogo" data-lang="ENG" ...>ENG</button>
    // <button id="korLogo" data-lang="KOR" ...>KOR</button>
    // <button id="cnLogo" data-lang="CN" ...>CN</button>
    // <button data-lang="none" ...>None</button>
    if (logoLangContainer) {
        logoLangContainer.addEventListener('click', (e) => {
            ...
            const textMap = { 'ENG': 'pictlord', 'KOR': 'ㅔㅑㅊ시ㅐㄱㅇ', 'CN': '照相亭', 'none': '' };
            customizationOptions.logo.text = textMap[lang];

            // Update UI (optional, jika Anda ingin mempertahankan highlight)
            logoLangContainer.querySelectorAll('button').forEach(btn => btn.classList.remove('active'));
    ...
    // Fungsi updateText tidak lagi diperlukan karena sudah ditangani oleh listener logo
    /*
    ...
    */
    ...
    // Fungsi ini tetap ada, tapi akan dipanggil dari dalam redrawCanvas
    function drawTextAndDate(ctx, canvas) {
        // Logika penentuan warna teks dipindahkan ke redrawCanvas agar lebih terpusat
        let textColor = customizationOptions.logo.color;
        
        if (customizationOptions.logo.lang !== 'none') {
    ...
    // FUNGSI drawSticker ANDA TETAP UTUH (SAYA SALIN SEMUA KODENYA)
    ...
    // FUNGSI clipAndDrawImage ANDA TETAP UTUH DAN TIDAK DIUBAH
    ...
    // SEMUA EVENT LISTENER UNTUK BENTUK FRAME ANDA TETAP UTUH DAN TIDAK DIUBAH
    ...
    function redrawCanvas() {
        ...
        // --- MODIFIKASI DIMULAI ---
        // Logika untuk menentukan warna teks berdasarkan latar belakang dan pilihan color picker
        let textColor = customizationOptions.logo.color;
        // --- MODIFIKASI SELESAI ---

        // Menggambar teks logo
        if (customizationOptions.logo.lang !== 'none') {
            ctx.fillStyle = textColor;
            ctx.font = 'bold 32px Arial, Roboto, sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText(customizationOptions.logo.text, stackedCanvas.width / 2, stackedCanvas.height - 55);
        }

        // --- MODIFIKASI DIMULAI ---
        // Menggambar tanggal dan waktu berdasarkan state dari `customizationOptions`
        if (customizationOptions.date.show || customizationOptions.date.showTime) {
            ...
            if (customizationOptions.date.showTime) {
                ...
                displayText += customizationOptions.date.show ? ' ' + timeString : timeString;
            }
    ...
    function updatePreview(canvas) {
        if (!canvas) {
            // Jika canvas tidak diberikan, gambar ulang dari awal
            redrawCanvas(); // Fungsi ini akan memanggil updatePreview lagi setelah selesai
            return;
        }
        ...
        if (photoCustomPreview) {
            ...
            if(backgroundColor == '#FFFFFF') { // menggunakan backgroundColor global karena ini hanya untuk style border
    ...
    // FUNGSI downloadCanvasCopy ANDA TETAP UTUH DAN TIDAK DIUBAH (HANYA MODIFIKASI KECIL PADA PEMANGGILAN LOGO & TANGGAL)
    async function downloadCanvasCopy() {
        ...
        // --- MODIFIKASI DIMULAI ---
        // Logika tanggal dan waktu disalin dari redrawCanvas
        let textColor = customizationOptions.logo.color;
        if (customizationOptions.date.show || customizationOptions.date.showTime) {
            const currentDate = new Date();
            let displayText = '';
        ...
        if (storedImages && storedImages.length === imageArrayLength) {
            ...
            storedImages.forEach((imgData, index) => {
                ...
                img.onload = async function () {
                    ...
                    if (loadedImages === imageArrayLength) {
                        // --- MODIFIKASI DIMULAI ---
                        // Memanggil drawTextAndDate dengan argumen yang benar
